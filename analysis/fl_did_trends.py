...
def calculate_ci(data, confidence=0.95):
    ...
    mean = np.mean(data)
    se = stats.sem(data)
    ci = se * stats.t.ppf((1 + confidence) / 2.0, n - 1)
...
def prepost_and_did_plot(
    ...
    state_year = (
        df.groupby(["state_abbrev", "year"])[outcome]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
    ...
    ctrl_agg = (
        ctrl.groupby("year")
        .agg({"mean": "mean", "ci_lower": "mean", "ci_upper": "mean"})
        .reset_index()
    )
    ...
    ax.fill_between(
        fl["year"], fl["ci_lower"], fl["ci_upper"], color="#d62728", alpha=0.2, zorder=2
    )
    ...
    ax.set_title(
        title_prefix, fontsize=12, fontweight="bold", pad=20
    )  # pad=20 for spacing
