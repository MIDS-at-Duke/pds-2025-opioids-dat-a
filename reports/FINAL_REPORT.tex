...
\textbf{Florida's regulatory approach produced sustained reductions.} Opioid shipments (measured in morphine milligram equivalents, or MME) dropped by 15 million per 100,000 residents immediately, and the decline accelerated over time. Drug deaths fell by 2.25 per 100,000 people. Both effects are statistically significant and sustained through 2015.
...
\caption{\textbf{Florida Opioid Shipments Dropped Sharply After 2010 Policy.} Red line shows Florida; blue dashed line shows 6 control states (GA, AL, SC, NC, TN, MS). Vertical line marks 2010. Immediate drop: 15.1M MME per 100k (p=0.004). Accelerating decline: $-23.7$M per year (p$<$0.001).}
\end{figure}
...
\caption{\textbf{Florida Drug Deaths Declined While Control States Rose.} Red line shows Florida; blue dashed shows controls. After 2010, Florida plateaus/declines while controls climb. Effect: $-2.25$ deaths per 100k (p=0.001) $\approx$ 430 lives saved annually.}
\end{figure}

\vspace{0.5cm}

\noindent Florida saw \textbf{2.25 fewer drug deaths per 100,000 residents} (p=0.001). For context, Florida's 2010 population was ~19 million, translating to roughly \textbf{430 lives saved per year}.
...
\caption{\textbf{Washington's Reduction Was Temporary.} Red: Washington; blue dashed: 6 controls (OR, CO, MN, NV, CA, VA). Initial drop: $-9.3$M MME (p=0.028), then \textcolor{red}{upward reversal: $+7.2$M per year} (p=0.001). Effect evaporated by 2015.}
\end{figure}
...
\textbf{Florida:} 83 counties (FL + 6 controls), 830 county-year observations
...
\midrule
Florida & Shipments (MME/100k) & $-15.1$M & 0.004** & $-23.7$M & $<$0.001*** \\
Florida & Deaths (per 100k) & $-2.25$ & 0.001** & $-0.82$ & 0.119 \\
\midrule
Washington & Shipments (MME/100k) & $-9.3$M & 0.028* & $+7.2$M & 0.001** \\
Washington & Deaths (per 100k) & $-1.12$ & 0.120 & $-0.05$ & 0.923 \\
\bottomrule
...
We re-estimated Florida's DiD using only three southeastern controls (GA, AL, SC) versus all six controls (GA, AL, SC, NC, TN, MS). Results: shipment reduction $-9.3$M (p=0.072) with 3 controls vs. $-15.1$M (p=0.004) with 6 controls. The weaker effect with fewer controls suggests our main findings benefit from the broader control group, though the effect remains in the expected direction. This highlights the importance of using multiple control states to improve statistical power.

\textbf{2. Population-Weighted Analysis}

Our main analysis uses county fixed effects with implicit equal weighting. We re-ran regressions weighting by county population to give more influence to larger counties. Florida's mortality effect strengthened: $-3.00$ per 100k (p$<$0.001) weighted vs. $-2.25$ (p=0.001) unweighted. This confirms results are not driven by small counties and, if anything, are \textit{stronger} in population-weighted specifications.

\textbf{3. Placebo Test (Pre-Policy Years)}

We tested for "effects" in 2008 (a year when no policy existed) using only 2006-2009 data. Results: MME effect p=0.001 (significant), deaths effect p=0.949 (not significant). The significant MME placebo effect suggests some pre-existing divergence in shipment trends between Florida and controls, which partially violates the parallel trends assumption for shipments. However, the mortality placebo test passes, supporting parallel trends for our primary outcome of interest (deaths).

\textbf{4. Exclusion of Border Counties}

To address potential spillover effects (e.g., Georgia residents crossing to Florida for prescriptions), we excluded 17 Florida counties bordering Georgia or Alabama. Results remained robust: shipments $-16.5$M (p=0.004), deaths $-2.20$ (p=0.004). The slightly stronger effects after exclusion suggest spillover is minimal and our main findings are not contaminated by cross-border dynamics.
